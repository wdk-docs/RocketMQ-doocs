...
.. note:: 目前 Exactly-Once 投递语义仅在 Java SDK 中支持。相关 SDK 下载，请参见 Java SDK 版本说明。

背景信息
----------------

消息队列 RocketMQ 的 Exactly-Once 投递语义适用于 **接收消息 > 处理消息 > 结果持久化到数据库** 的流程，
能够保证您的每一条消息消费的最终处理结果写入到您的数据库有且仅有一次，保证消息消费的幂等。
